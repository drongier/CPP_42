...
std::vector<int> extractWinners(const std::vector<Pair>& pairs) {
    ...
    for (size_t i = 0; i < pairs.size(); ++i) {
        ...
        // Si b == -1, c'est l'élément non apparié, à traiter à part
    }
    return winners;
}
...
int findUnpaired(const std::deque<Pair>& pairs) {
	if (pairs.empty())
		return -1;
...
std::vector<int> pmerge::fordJohnson(const std::vector<int>& input) {
	...
	// std::cout << "Winner vector: ";
	// printVector(winners);
	// std::cout << std::endl;
    std::vector<int> losers = extractLosers(pairs);
	// std::cout << "Loser vector: ";
	// printVector(losers);
	// std::cout << std::endl;
